...
class TelegramBot:
    ...
    def _should_add_contacts(self, question):
        """Определяет, нужно ли добавлять контактные данные (Правило 3)"""
        contact_keywords = [
    ...
    def _should_redirect_to_website(self, question):
        """Определяет, относится ли вопрос к стоимости (Правило 5)"""
        price_keywords = [
...
1. ССЫЛКИ НА САЙТ:
   ...
   - Желательно размещать ссылки в конце предложения, где они упоминаются
...
4. ОБРАЩЕНИЯ:
   - "Вам", "Вы", "Ваш" всегда пишутся с заглавной буквы, даже в середине предложения
   Пример: Для Вас мы подготовили специальные условия.
...
Я могу ответить на Ваши вопросы о программах смен, документах для заезда, мерах безопасности и многом другом.

Для связи с администрацией лагеря (тел. {self.contact_phone}) Вы можете позвонить в рабочее время или написать на email: {self.contact_email}
...
СТРОГО СОБЛЮДАЙ все правила форматирования из инструкции."""
            ...
            # Правило 5: Для вопросов о стоимости добавляем ссылку на сайт
